...
const reducerCreator = (action: IAction) => createReducer<IInitialState>(action, ({
    DEFAULT,
    SUCCESS,
}) => ({
    [DEFAULT]: (state: IInitialState, action: IActionObject) => ({ ...state, a: state.a + action.payload }),
    [SUCCESS]: (state: IInitialState, action: IActionObject) => ({ ...state, b: state.b + action.payload }),
}), initialState);
...
describe('Compose', () => {
    test('Handle simple reducers', () => {
        const rA = createSimpleReducer(actionA, reducerA, initialState);
        const rB = createSimpleReducer(actionB, reducerB, initialState);
        const reducer = compose(initialState, rA, rB);
    ...
    test('Handle reducers', () => {
        ...
        const reducer = compose(initialState, r1, r2);
    ...
    test('Handle mixed: simple reducer and reducer', () => {
        const r1 = reducerCreator(actionA);
        const rB = createSimpleReducer(actionB, reducerB, initialState);
        const reducer = compose(initialState, r1, rB);
