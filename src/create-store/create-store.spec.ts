...
describe('Configure store', () => {
    ...
    test('No options provided', () => {
        expect(createStore)
            .toThrowError('No options provided');
    });

    test('No reducer provided', () => {
        expect(() => createStore({ reducer: null }))
            .toThrowError('Reducer must be a function or an object that can be passed to combineReducers');
    });
    ...
    test('Middleware provided', () => {
        const reducer: redux.Reducer = (state = {}) => state;
        const middleware: redux.Middleware = store => next => action => next(action);

        expect(createStore({ middleware: [middleware], reducer }))
    ...
    test('Enhancers provided', () => {
        ...
        expect(createStore({ enhancers: [enhancer], reducer }))
            .toBeInstanceOf(Object);
        expect(redux.applyMiddleware).toHaveBeenCalled();
        expect(devtools.composeWithDevTools).toHaveBeenCalled();
        expect(redux.createStore).toHaveBeenCalledWith(
